...
def main():
    ...
    again = True
    while again:
        print("Choose difficulty level from 1 to 10 ")
        depth = int(input())

        board = [['0' for c in range(columns)] for r in range(rows)]
        print("Who would you like to go first? 1 for computer (Red) or 2 for you (Yellow)")
        Turn = int(input())
        ...
        while not game_over(board, columns, rows, connect4):
            if computerTurn:
                ...
                addmove(board, computerTurn, columns, rows)
